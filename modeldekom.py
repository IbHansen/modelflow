...
class totdekomp():
    ...
    def __init__(self, model,summaryvar='*',desdic={} ):
       ...
       if len(self.diffvar) == 0:
           ...
           self.res = attribution(self.model,self.experiments,self.start,self.end,summaryvar=self.summaryvar,showtime=1,silent=1)
    ...
    def explain_per(self,pat='',per='',top=0.9,title='Attribution, for one periode'):   
        if self.go:        
           tper = self.res.columns.get_level_values(1)[0] if per == '' else per
           self.impact = mk.GetOneImpact(self.res,pat=pat,per=tper).T
           ntitle = f'{title}: {tper}'
